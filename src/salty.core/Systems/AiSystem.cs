...
{
    [With(typeof(Transform2), typeof(IAiComponent), typeof(StateComponent))]
    public class AiSystem : AEntitySetSystem<float>
    {
        ...
        protected override void Update(float elapsedTime, in Entity entity)
        {
            ...
            var aiSystem = entity.Get<IAiComponent>();
            var state = entity.Get<StateComponent>();

            var oldPosition = transform.Position;
            var newPosition = aiSystem.Update(transform.Position, elapsedTime);
